...
void processFile(const fs::path& filePath) {
    try {
        JackCompiler compiler(filePath.string());
        compiler.writeVMCode();
        std::cout << "Compiled: " << filePath.filename() << "\n";
    } catch (const std::exception& e) {
        std::cerr << "Error compiling " << filePath << ": " << e.what() << "\n";
    }
}

int main(int argc, char* argv[]) {
    if (argc != 2) {
        std::cerr << "Usage: compiler <file.jack | directory>\n";
        return 1;
    }
    ...
    std::cout << "Done. Compiled " << jackFiles.size() << " file(s) to VM code.\n";
    return 0;
}