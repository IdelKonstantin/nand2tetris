...
void processFile(const fs::path& filePath) {
    try {
        JackTokenizer tokenizer(filePath.string());
        tokenizer.writeTokensToXML();
        std::cout << "Tokenized: " << filePath.filename() << "\n";
...
int main(int argc, char* argv[]) {
    if (argc != 2) {
        std::cerr << "Usage: tokenizer <file.jack | directory>\n";
        return 1;
    }
    ...
    if (fs::is_regular_file(inputPath) && inputPath.extension() == ".jack") {
        // Один файл — обработка в текущем потоке
